	...
	: fSettings(settings), fWriteFragmentTree(writeFragmentTree), fFragmentTreeEntries(0), fRunNumber(runNumber), fSubRunNumber(subRunNumber), fRunInfo(runInfo), fKValue(settings->KValue())
{
	...
	// RCMP/QED
	fRcs = new TRcmp;
	fEventTree.Branch("TRcmp", &fRcs, fSettings->BufferSize());
	
	//fRcsN = new TRcmp;
	//fEventTree.Branch("TRcmp", &fRcsN, fSettings->BufferSize());
...
bool Converter::Run() {
	...
	for(int i = 0; i < nEntries; ++i) {
		...
		if(fSystemID >= 2000 &&fSystemID < 9000) {
			fCryNumber = 0;
		}
		...
		if((fSettings->SortNumberOfEvents()==0)||(fSettings->SortNumberOfEvents()>=fEventNumber) ) {
			...
			if(AboveThreshold(smearedEnergy, fSystemID)) {
				if(InsideTimeWindow() ) {
					switch(fSystemID) {
						//mapping systems to address ranges: 0 - GRIFFIN, 1 - BGO, 2 - LaBr, 3 - ancilliary BGO, 4 - NaI, 5 - SCEPTAR, 6 - SPICE, 7 - PACES, 8 - DESCANT, 9-QED
						case 1000://griffin
					...
					if(fSystemID==9000){
						...
						//if(fDetNumber>=0 && fDetNumber<1024){
							...
							//detector number have default starts with 1, set it back to 0.
							fDetNumber-=1;
							fRCSDetNumber=fDetNumber/1024+1;
							//std::cout<<fDetNumber<<std::endl;
						//}
						
						
						if(fDetNumber==0){
						
							std::cout<<"found detector 0"<<std::endl;
						
						}
						
						address = 9000 +(fDetNumber-(fRCSDetNumber-1)*1024)/32+(fRCSDetNumber-1)*32 ;
						address2 = 9200 +(fRCSDetNumber-1)*32 +(fDetNumber-1024*(fRCSDetNumber-1))%32;
					...
					if(fFragments.count(address) == 1) {
						...
						if(fSystemID==9000){
						fFragments[address2].SetCharge(fFragments[address].GetCharge()+smearedEnergy*fKValue);
						// update timestamp
						fFragments[address2].SetTimeStamp(fTime*1e8);
						...
						// if this is RCMP/QED hits, also set the fragments for N side detector channels
						
						if(fSystemID==9000){
						fFragments[address2].SetAddress(address2);
						//fFragments[address].SetCcLong();
						//fFragments[address].SetCcShort();
						fFragments[address2].SetCfd(0);
						fFragments[address2].SetCharge(smearedEnergy*fKValue);
						
						fFragments[address2].SetKValue(fKValue);
						//fFragments[address].SetMidasId(fFragmentTreeEntries);
						// fTime is the time from the beginning of the event in seconds
						fFragments[address2].SetDaqTimeStamp(fTime); 
						fFragments[address2].SetTimeStamp(fTime*1e8);
						//fFragments[address].SetZc();
						++fFragmentTreeEntries;
						...
						if(channel == nullptr ||channel2 == nullptr) {
							...
							switch(fSystemID) {
								...
								case 9000:{
									...
									//based on detector pixel construction of QED set up, assigning all Cols to Junction strips
									
									uint32_t address = 9000 +std::floor((fDetNumber-(fRCSDetNumber-1)*1024)/32)+(fRCSDetNumber-1)*32 ;
									
									std::cout<<"Strip number is "<<std::floor((fDetNumber-(fRCSDetNumber-1)*1024)/32)<<std::endl;
									std::cout<<fDetNumber<<std::endl;
        								channel = new TChannel;
        								//!!!!!!!!!!!!!!!!!Using detector number for memomnic!!!!!!!!!!!!!! All detector have channel1
        								mnemonic  = Form("RCS%02dXP%02dX",fRCSDetNumber,int((fDetNumber-(fRCSDetNumber-1)*1024)/32));
        								//Not sure why it doesn't work with detector channel number in memonic
        								//mnemonic  = Form("RCS01XP%02dX", int(fDetNumber/32));
        								digitizerType = "FMC32";
        								...
        								//channel->SetDetectorNumber(int(fDetNumber/32));
        								channel->SetDetectorNumber(fRCSDetNumber);

        								channel->SetCrystalNumber(std::floor((fDetNumber-(fRCSDetNumber-1)*1024)/32));
									...
									if(channel2 == nullptr){
									std::cout<<int(fDetNumber%32)<<std::endl;
									//All Ohmic strips begins with address 9200, for 6 detector setup Ohmic side has 32*6 strips
									//Address should end in 9192 
									uint32_t address2 = 9200 +(fRCSDetNumber-1)*32 +(fDetNumber-1024*(fRCSDetNumber-1))%32;
								
								
									channel2 = new TChannel;
									//Ohmic side
									mnemonic2 = Form("RCS%02dXN%02dX",fRCSDetNumber, (fDetNumber-1024*(fRCSDetNumber-1))%32);
									std::cout<<"Ohmic"<<std::endl;
									std::cout<<fDetNumber<<std::endl;
									std::cout<<mnemonic2<<std::endl;
									//digitizerType = "FMC32";
									channel2->SetAddress(address2);
									channel2->SetName(mnemonic2.c_str());
									channel2->SetDetectorNumber(fRCSDetNumber);
									channel2->SetCrystalNumber((fDetNumber-1024*(fRCSDetNumber-1))%32);
									channel2->SetDigitizerType(TPriorityValue<std::string>(digitizerType, EPriority::kRootFile));
									TChannel::AddChannel(channel2);
									fFragments[address2].SetCfd(Cfd(EDigitizer::kGRF16));
								}
...
void Converter::FillDetectors() {
	for(auto frag : fFragments) {
		...
		switch(frag.second.GetAddress()/1000) {
			...
			case 9:
				fRcs->AddFragment(std::make_shared<TFragment>(frag.second), channel);
				if(fSettings->VerbosityLevel() > 2) {
					std::cout<<"Added fragment "<<fFragment<<" to rcmp:"<<std::endl;
					fFragment->Print();
				}
				break;
