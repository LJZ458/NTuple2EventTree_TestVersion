...
Settings::Settings(std::string fileName, int verbosityLevel)
    ...
    // Paces
    fResolution[50].resize(17);
    fThreshold[50].resize(17,std::vector<double>(1));
    fThresholdWidth[50].resize(17,std::vector<double>(1));
    fTimeWindow[50].resize(17,std::vector<double>(1));
    
    
    
    // RCMP/QED P
    fResolution[9000].resize(400);
    fThreshold[9000].resize(400,std::vector<double>(32));
    fThresholdWidth[9000].resize(400,std::vector<double>(32));
    fTimeWindow[9000].resize(400,std::vector<double>(32));
    
    // RCMP/QED N
    //fResolution[9200].resize(16);
    //fThreshold[9200].resize(16,std::vector<double>(1));
    //fThresholdWidth[9200].resize(16,std::vector<double>(1));
    //fTimeWindow[9200].resize(16,std::vector<double>(1));
    ...
    for(int detector = 0; detector < 16; ++detector) {
        for(int crystal = 0; crystal < 4; ++crystal) {
            offset = env.GetValue(Form("Griffin.%d.%d.Resolution.Offset",detector,crystal),0.00);
            linear = env.GetValue(Form("Griffin.%d.%d.Resolution.Linear",detector,crystal),0.00);
            quadratic = env.GetValue(Form("Griffin.%d.%d.Resolution.Quadratic",detector,crystal),0.000);
            ...
            fThreshold[1000][detector][crystal] = env.GetValue(Form("Griffin.%d.%d.Threshold.keV",detector,crystal),0.);
            fThresholdWidth[1000][detector][crystal] = env.GetValue(Form("Griffin.%d.%d.ThresholdWidth.keV",detector,crystal),0.);
    ...
    for(int detector = 0; detector < 16; ++detector) {
        ...
        linear = env.GetValue(Form("LaBr3.%d.Resolution.Linear",detector),0.);
        quadratic = env.GetValue(Form("LaBr3.%d.Resolution.Quadratic",detector),0.);
    ...
    // Paces
    for(int detector = 0; detector < 17; ++detector) {

        offset = env.GetValue(Form("Paces.%d.Resolution.Offset",detector),5.0);
        linear = env.GetValue(Form("Paces.%d.Resolution.Linear",detector),0.002);
        ...
        fResolution[50][detector].push_back(TF1(Form("Paces.%d.Resolution",detector),
                                                  Form("(TMath::Sqrt((%f+%f*x+%f*x*x+%f*x*x*x)))/(2.*TMath::Sqrt(2.*TMath::Log(2.)))",offset, linear, quadratic, cubic),0.,100000.));
        //fThreshold[50][detector][0] = env.GetValue(Form("Paces.%d.Threshold.keV",detector),0.);
        //fThresholdWidth[50][detector][0] = env.GetValue(Form("Paces.%d.ThresholdWidth.keV",detector),0.);
        //fTimeWindow[50][detector][0] = env.GetValue(Form("Paces.%d.TimeWindow.sec",detector),0.);
    }
    ...
    //QED 
        ...
        for(int crystal = 0; crystal < 32; ++crystal) {
	//pside
     /*   offset = env.GetValue(Form("QedP.%d.Resolution.Offset",detector),0.0);
        linear = env.GetValue(Form("QedP.%d.Resolution.Linear",detector),0.002);
        quadratic = env.GetValue(Form("QedP.%d.Resolution.Quadratic",detector),0.0);
        cubic = env.GetValue(Form("QedP.%d.Resolution.Cubic",detector),0.0);
        fResolution[9000][detector].push_back(TF1(Form("QedP.%d.Resolution",detector),
                                             Form("(TMath::Sqrt((%f+%f*x+%f*x*x+%f*x*x*x)))/(2.*TMath::Sqrt(2.*TMath::Log(2.)))",offset, linear, quadratic, cubic),0.,100000.));
       */ 
        offset = env.GetValue(Form("QedP.%d.%d.Resolution.Offset",detector,crystal),0.00);
            linear = env.GetValue(Form("QedP.%d.%d.Resolution.Linear",detector,crystal),0.00);
            quadratic = env.GetValue(Form("QedP.%d.%d.Resolution.Quadratic",detector,crystal),0.000);
            cubic = env.GetValue(Form("QedP.%d.%d.Resolution.Cubic",detector,crystal),0.);
            fResolution[9000][detector].push_back(TF1(Form("QedP.%d.%d.Resolution",detector,crystal),
                                                      Form("(TMath::Sqrt((%f+%f*x+%f*x*x+%f*x*x*x)))/(2.*TMath::Sqrt(2.*TMath::Log(2.)))",offset, linear, quadratic, cubic),0.,100000.));
            fThreshold[9000][detector][crystal] = env.GetValue(Form("QedP.%d.%d.Threshold.keV",detector,crystal),0.);
            fThresholdWidth[9000][detector][crystal] = env.GetValue(Form("QedP.%d.%d.ThresholdWidth.keV",detector,crystal),0.);
            fTimeWindow[9000][detector][crystal] = env.GetValue(Form("QedP.%d.%d.TimeWindow.sec",detector,crystal),0.);
 ...
 /*       for(int detector = 0; detector < 193; ++detector) {
        
        //nside               
        offset = env.GetValue(Form("QedN.%d.Resolution.Offset",detector),0.0);
        linear = env.GetValue(Form("QedN.%d.Resolution.Linear",detector),0.002);
        quadratic = env.GetValue(Form("QedN.%d.Resolution.Quadratic",detector),0.0);
        cubic = env.GetValue(Form("QedN.%d.Resolution.Cubic",detector),0.0);
        fResolution[9000][detector+200].push_back(TF1(Form("QedN.%d.Resolution",detector),
                                                  Form("(TMath::Sqrt((%f+%f*x+%f*x*x+%f*x*x*x)))/(2.*TMath::Sqrt(2.*TMath::Log(2.)))",offset, linear, quadratic, cubic),0.,100000.));                                        
